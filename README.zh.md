...
## 功能
1. 可以通过`Diff`面板审查 Agent 刚刚修改的每个文件，并且同时支持标准模式和PTC模式。
2. 支持撤销操作，目前支持在标准模式、PTC模式、创造模式下对Agent执行的文件修改、新增文件等操作进行撤销。
    > 因为目前dsh暂时没有提供删除文件的相关tool，因此本插件暂时不支持对删除的文件进行撤销操作。后续若dsh提供了删除文件的tool，本插件会进一步地支持。
3. 可对变更行添加评论，并让 Agent 根据评论内容继续修改，或者对改变更进行询问。
4. 可在“设置 → 插件 → 插件配置 → 文件审查”中选择是否对过长的差异行自动换行。换行只影响显示，不改变复制出的文本及其实际行结构，默认关闭。
5. 多语言支持，包括中文和英文。
## 快速开始
